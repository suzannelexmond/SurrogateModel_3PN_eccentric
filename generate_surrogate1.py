    ...
    def fit_to_training_set(self, property, min_greedy_error, save_fits_to_file=False, plot_kernels=False, plot_fits=False, save_fig_kernels=False, save_fig_fits=False):
        ...
        def gaussian_process_regression(time_node, training_set, plot_kernels=plot_kernels, save_fig_kernels=save_fig_kernels):
            ...
            kernels = [
                ...
                C(1.0, (1e-4, 1e2)) * Matern(length_scale=5.0, length_scale_bounds=(1e-4, 1), nu=1.5)
...
print(np.linspace(0.01, 0.2, num=500).round(4))
gs = Generate_Surrogate(parameter_space=np.linspace(0.01, 0.2, num=500).round(4), min_greedy_error_amp=1e-2, min_greedy_error_phase=1e-3, waveform_size=3500, freqmin=18)
gs.fit_to_training_set(property='phase', min_greedy_error=1e-3, plot_fits=True, save_fits_to_file='GPRfits_0.01_0.2')
...
gs.generate_surrogate_model(plot_surr_datapiece_at_ecc=0.0218, plot_surr_at_ecc=0.0218)

...
class Generate_Surrogate(Generate_TrainingSet):
    ...
    def __init__(self, parameter_space, min_greedy_error_amp, min_greedy_error_phase, waveform_size=None, total_mass=50, mass_ratio=1, freqmin=20):
        ...
        Generate_TrainingSet.__init__(self, parameter_space=parameter_space, waveform_size=waveform_size, total_mass=total_mass, mass_ratio=mass_ratio, freqmin=freqmin)
    ...
    def fit_to_training_set(self, property, min_greedy_error, save_fits_to_file=False, plot_kernels=False, plot_fits=False, save_fig_kernels=False, save_fig_fits=False):
        ...
        def gaussian_process_regression(time_node, training_set, plot_kernels=plot_kernels, save_fig_kernels=save_fig_kernels):
            ...
            X_train = np.array(self.parameter_space[self.greedy_parameters_idx]).reshape(-1, 1)
            ...
            kernels = [
                ...
                C(1.0, (1e-4, 1e2)) * Matern(length_scale=5.0, length_scale_bounds=(1e-3, 1e3), nu=1.5)
            ...
            if plot_kernels is True:
                ...
                if save_fig_kernels is True:
                    figname = 'Gaussian kernels {property} M={}, q={}, ecc=[{},{}].png'.format(self.total_mass, self.mass_ratio, min(self.parameter_space), max(self.parameter_space))
        ...
        try:
            load_GPRfits = np.load(f'Straindata/GPRfits/{property}_{min(self.parameter_space)}_{max(self.parameter_space)}.npz')
            gaussian_fit = load_GPRfits['GPRfit']
            ...
            gaussian_fit = np.zeros((len(training_set.T), len(self.parameter_space)))
            uncertainty_region = []
        ...
        if plot_fits is True:
            ...
            for i in range(len(gaussian_fit)):
                ...
                axs[0].plot(self.parameter_space, gaussian_fit.T[:, i], color=colors[i], linewidth=0.6)
                axs[0].scatter(self.parameter_space[self.greedy_parameters_idx], training_set[:, i], color=colors[i])
                axs[0].plot(self.parameter_space, residual_parameterspace[:, self.empirical_nodes_idx[i]], linestyle='dashed', linewidth=0.6, color=colors[i], label=f't={self.TS_M[self.empirical_nodes_idx[i]]}')
                
                # relative_error = abs(residual_parameterspace[:, self.empirical_nodes_idx[i]] - gaussian_fit.T[:, i]) / residual_parameterspace[:, self.empirical_nodes_idx[i]]
                relative_error = abs(residual_parameterspace[:, self.empirical_nodes_idx[i]] - gaussian_fit.T[:, i]) / residual_parameterspace[:, self.empirical_nodes_idx[i]]
                axs[1].plot(self.parameter_space, relative_error, color=colors[i], label=f'Error {i+1} (t={int(self.TS_M[self.empirical_nodes_idx[i]])})')
            ...
            if save_fig_fits is True:
                figname = 'Gaussian fits {property} M={}, q={}, ecc=[{},{}].png'.format(self.total_mass, self.mass_ratio, min(self.parameter_space), max(self.parameter_space))
        ...
        if save_fits_to_file is not None and not os.path.isfile(f'Straindata/GPRfits/{save_fits_to_file}'):
            ...
            np.savez(f'Straindata/GPRfits/{property}_{min(self.parameter_space)}_{max(self.parameter_space)}.npz', GPR_fit=gaussian_fit)
            print('GPR fits saved')

        return gaussian_fit, uncertainty_region

    def generate_surrogate_model(self, plot_surr_datapiece_at_ecc=False, save_fig_datapiece=False, plot_surr_at_ecc=False, save_fig_surr=False):
        ...
        def reconstruct_surrogate_datapiece(property, B_matrix, fit_matrix, plot_surr_datapiece_at_ecc=plot_surr_datapiece_at_ecc, save_fig_datapiece=save_fig_datapiece):
            ...
            reconstructed_residual = np.zeros((len(self.TS_M), len(self.parameter_space)))
            ...
            print('shape surr', surrogate_datapiece.shape)
            ...
            if isinstance(plot_surr_datapiece_at_ecc, float):
                ...
                print('dp')
                if property == 'amplitude':
                    real_datapiece = np.array(waveform.utils.amplitude_from_polarizations(real_hp, real_hc))
                    units = ''
                elif property == 'phase':
                ...
                print('dp 2')
                try:
                    index_ecc_min = np.where(self.parameter_space == plot_surr_datapiece_at_ecc)[0][0]
                except:
                    print('Eccentricity value not in parameterspace')
                    sys.exit(1)
                ...
                axs[0].plot(self.TS_M, real_datapiece[length_diff:], linewidth=0.6, linestyle='dashed', label=f'Real: e = {plot_surr_datapiece_at_ecc}')
                ...
                relative_error = abs(surrogate_datapiece.T[index_ecc_min] - real_datapiece[length_diff:]) / real_datapiece[length_diff:]
                ...
                if save_fig_datapiece is True:
                    figname = f'Surrogate_{property}_eccmin={plot_surr_datapiece_at_ecc}_for_ecc=[{min(self.parameter_space)}_{max(self.parameter_space)}].png'
        ...
        fit_matrix_amp = self.fit_to_training_set(min_greedy_error=self.min_greedy_error_amp, property='amplitude' )[0]
        ...
        # Get matrix with interpolated fits and B_matrix
        fit_matrix_phase = self.fit_to_training_set(min_greedy_error=self.min_greedy_error_phase, property='phase' )[0]
        ...
        h_surrogate = np.zeros((len(self.TS_M), len(self.parameter_space)), dtype=complex)
        ...
        if isinstance(plot_surr_at_ecc, float):
            ...
            axs[0].plot(self.TS_M, true_hp[length_diff:], linewidth=0.6, label='Surrogate')
            axs[0].plot(self.TS_M, np.real(h_surrogate[:, index_ecc_min]), linewidth=0.6, label='True waveform')
            axs[0].set_ylabel('$h_+$')
            axs[0].grid(True)
            axs[0].legend()
            ...
            axs[2].plot(self.TS_M, true_hc[length_diff:], linewidth=0.6, label='Surrogate')
            axs[2].plot(self.TS_M, np.imag(h_surrogate[:, index_ecc_min]), linewidth=0.6, label='True waveform')
            axs[2].grid(True)
            axs[2].legend()
            ...
            if save_fig_surr is True:
                figname = f'Surrogate_wf_eccmin={plot_surr_at_ecc}_for_ecc=[{min(self.parameter_space)}_{max(self.parameter_space)}].png'
...
gs = Generate_Surrogate(parameter_space=np.linspace(0.01, 0.2, num=500).round(4), min_greedy_error_amp=1e-2, min_greedy_error_phase=1e-2, waveform_size=3500, freqmin=18)
gs.fit_to_training_set(property='phase', min_greedy_error=1e-2, plot_fits=True, save_fits_to_file='GPRfits_0.01_0.2')
gs.fit_to_training_set(property='amplitude', min_greedy_error=1e-2, plot_fits=True, save_fits_to_file='GPRfits_0.1_0.2')

# ecc_list = np.linspace(0.01, 0.1, num=11).round(4)

# gs.generate_property_dataset(eccmin_list=ecc_list, property='phase', plot_residuals=True)
# fig1 = plt.figure()
# 'residual_phase_full_parameterspace_0.1_0.2.npz'

gs.generate_surrogate_model(plot_surr_datapiece_at_ecc=0.2, plot_surr_at_ecc=0.2)

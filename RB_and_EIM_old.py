...
class Reduced_basis(Dataset):
    ...
    def import_polarisations(self, save_dataset=False, eccmin_list=None):
        try:
            ...
            print(hp_DS.shape, hc_DS.shape, self.TS_M.shape)
    ...
    def import_waveform_property(self, property='Phase', save_dataset=False, eccmin_list=None, val_vecs=False):
        try:
            if property == 'Amplitude' or property == 'Phase':
                ...
                if val_vecs is True:
                    ...
                    residual_dataset = np.loadtxt(f'Straindata/Valvecs_Res_{property}_{min(self.eccmin_list)}_{max(self.eccmin_list)}.txt', skiprows=1)
                    self.TS_M = np.loadtxt(f'Straindata/Valvecs_TS_{min(self.eccmin_list)}_{max(self.eccmin_list)}.txt', skiprows=1)[-self.waveform_size:]
            ...
            if property == 'Amplitude' or property == 'Phase':
                residual_dataset = self.generate_dataset_property(property, save_dataset, eccmin_list, val_vecs)
    ...
    def strong_greedy(self, U, N, parameter_list, reg=1e-6):
        """
        Perform strong greedy selection to arrange the training set from least similar to most similar. 

        Parameters:
        - U (numpy.ndarray): NORMALIZED training set, each row represents a data point.
        ...
        # In some cases this is a singular matrix and will cause computational problems. To prevent this, I added a small regulation to the diagonal terms of the matrix.
            ...
            parameters.append(parameter_list[np.argmax(errors)])
        
        return ordered_basis, parameters

    def plot_greedy_error(self, num_validation_vecs, property='Phase', save_dataset='False'):

        training_set, TS_M = self.import_waveform_property(property=property, save_dataset=True, val_vecs=True)
        validation_vecs = self.calc_validation_vectors(num_validation_vecs, property)

        training_set = training_set / np.linalg.norm(training_set, axis=1, keepdims=True)
        validation_vecs = validation_vecs / np.linalg.norm(validation_vecs, axis=1, keepdims=True)
        print('TS', training_set.shape)
        greedy_basis, parameters_gb = self.strong_greedy(training_set, len(training_set), self.eccmin_list)
        ...
        N_basis_vectors = np.linspace(0, len(trivial_errors), num=(len(trivial_errors)))
...
class Empirical_Interpolation_Method(Reduced_basis, Dataset):
    ...
    def plot_empirical_nodes(self, polarisation='plus', save_dataset = False):
        ...
        plt.plot(self.TS_M, hp_DS[-1], linewidth=0.2, label = 'e = {}'.format(self.eccmin_list[-1]))
...
RB.plot_greedy_error(num_validation_vecs=5, property='Phase')
RB.plot_greedy_error(num_validation_vecs=5, property='Amplitude')
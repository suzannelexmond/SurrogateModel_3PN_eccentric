...
class Generate_TrainingSet(Waveform_properties, Simulate_Inspiral):
    ...
    def generate_property_dataset(self, eccmin_list, property, save_dataset_to_file=None, plot_residuals=False, save_fig=False):
        ...
        #     residual_dataset[:, i] += self.phase_shift_total
        ...
        # print(residual_dataset.shape, self.phase_shift_total.shape)
        # residual_dataset += self.phase_shift_total[:, None]
...
gds = Generate_TrainingSet(parameter_space=np.linspace(0.01, 0.2, num=500).round(4), waveform_size=3500, freqmin=18)
...
# parameter_space=np.linspace(0.01, 0.2, num=50).round(4)
# load = np.load('Straindata/Residuals/residual_phase_full_parameterspace_0.01_0.2_noshift.npz')
# residuals = load['residual']
# print(residuals.shape)

# fig1 = plt.figure()
# plt.plot(parameter_space, residuals[:, 20])
# plt.plot(parameter_space, residuals[:, 30])
# plt.plot(parameter_space, residuals[:, 40])
# plt.show()

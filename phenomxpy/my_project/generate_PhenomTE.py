...
class Simulate_Inspiral:
    ...
    def __init__(self, time_array, ecc_ref, total_mass, luminosity_distance, f_lower=10, f_ref=20, chi1=0, chi2=0, phiRef=0., rel_anomaly=0., inclination=0., truncate_at_ISCO=True):
        """
        Parameters:
        ----------------
    ...
    def truncate_waveform_at_isco(self, phen, plot_ISCO_cut_off=True):
        ...
        if plot_ISCO_cut_off is True:
            ...
            plt.plot(self.time, Mf, label=f'Mf before ISCO cut: $e$={round(phen.pWF.eccentricity, 2)}')
            ...
            plt.plot(self.time[:idx_cut], Mf[:idx_cut], label=f'Mf after ISCO cut: $e$={round(phen.pWF.eccentricity, 2)} ')
            plt.legend()
    ...
    def simulate_inspiral_mass_independent(self, ecc_ref=None, plot_polarisations=False, save_fig=False, truncate_at_ISCO=False):
        ...
        print(f'time : SimInspiral_M_independent ecc = {round(ecc_ref, 3)}, M = {self.total_mass}, f_lower = {self.f_lower}', (timer()-start), ' seconds')
        ...
        if plot_polarisations is True:
            ...
            plt.xlabel('t [s]')
            plt.ylabel('$h_{22}]$')
            plt.title(f'M={self.total_mass}, e={round(ecc_ref, 2)}, f_min={self.f_lower} Hz')
            plt.grid(True)

            plt.tight_layout()

            if save_fig is True:
                figname = 'Polarisations_M={}_ecc={}.png'.format(self.total_mass, round(ecc_ref, 2))
...
class Waveform_Properties(Simulate_Inspiral):
    ...
    def __init__(self, time_array, ecc_ref, total_mass, luminosity_distance, f_lower=10, f_ref=20, chi1=0, chi2=0, phiRef=0., rel_anomaly=0., inclination=0., truncate_at_ISCO=True):
        ...
        Simulate_Inspiral.__init__(self, time_array, ecc_ref, total_mass, luminosity_distance, f_lower, f_ref, chi1, chi2, phiRef, rel_anomaly, inclination, truncate_at_ISCO)
    ...
    def calculate_residual(self, hp, hc, ecc_ref, property=None, plot_residual=False, save_fig=False):
        ...
        if property == 'phase':
            circ = self.phase(self.hp_circ, self.hc_circ) # non-eccentric case
        ...
        # Calculate amplitude from plus and cross polarisations
            ...
            circ = self.amplitude(self.hp_circ, self.hc_circ) # non-eccentric case
        ...
        if plot_residual is True:
            ...
            if save_fig is True:
                figname = f'Residual {property} M={self.total_mass}, ecc={round(ecc_ref, 2)}.png'
...
# wp = Waveform_Properties(time_array=time_array, ecc_ref=0.2, total_mass=60, luminosity_distance=200)
# hp, hc = wp.simulate_inspiral_mass_independent(truncate_at_ISCO=True, plot_polarisations=True, save_fig=True)

...
# Check version
print(f"gw_eccentricity version: {gwe.__version__}")

# See what's available
print(f"Available functions: {[attr for attr in dir(gwe) if not attr.startswith('_')]}")
        ...
        if truncate_at_tmin:
            if phen.pWF.tmin > time_array[0]:
                warnings.warn(self.colored_text(
                    "t_min is larger than parts of the specified time-domain, resulting in unphysical waveforms. The time-domain has been cut to physical length!"
...
class Waveform_Properties(Simulate_Waveform):
    ...
    def measure_eccentricity_from_h22(self, make_diagnostic_plots=False):
        """Uses Amplitude method - NO circular reference required."""
        import gw_eccentricity as gwe
        ...
        dataDict = {
            "t": self.time,
            "hlm": {(2, 2): h22_ecc}
            # NO "t_zeroecc" or "hlm_zeroecc" needed for Amplitude method!
        }
        
        # 2. Use "Amplitude" method (NOT ResidualAmplitude!)
        return_dict = gwe.measure_eccentricity(
            tref_in=time_circ_ext,
            method='Amplitude',        # ← Key difference!
            dataDict=dataDict
        )
        
        # 3. Extract results
        gwecc_object = return_dict["gwecc_object"]
        self.mean_anomaly = gwecc_object.mean_anomaly
        self.eccentricity = gwecc_object.eccentricity
        
        if make_diagnostic_plots:
            gwecc_object.make_diagnostic_plots()
        
        return self.eccentricity, self.mean_anomaly
    ...
    def get_orbital_parameters(self, plot_orbital_parameters=False, save_fig_orbital_parameters=False, make_diagnostic_plots=False):
        ...
        dataDict = {"t": self.time,
           "hlm": {(2, 2): h22_ecc},
           "t_zeroecc": time_circ_ext, # Must be longer than ecc to avoid extrapolation errors in gw_eccentricity package
        ...
        return_dict = gwe.measure_eccentricity(tref_in=tref_in,
                                            method=method,
                                            dataDict=dataDict)
...
# wp = Waveform_Properties(
#     time_array = time_array,
#     ecc_ref=0.2,
#     mean_anomaly_ref=np.pi,
#     total_mass=60,
...
# wp.measure_eccentricity_from_h22(
#     # plot_orbital_parameters=True,

...
class Generate_Surrogate(Generate_TrainingSet):
    ...
    def fit_to_training_set(self, property, min_greedy_error=None, N_greedy_vecs=None, save_fits_to_file=True, plot_kernels=False, plot_fits=False, save_fig_kernels=False, save_fig_fits=False, plot_residuals_ecc_evolve=False, save_fig_ecc_evolve=False, plot_residuals_time_evolve=False, save_fig_time_evolve=False):
        ...
        if save_fits_to_file is True and not os.path.isfile(filename):
            ...
            np.savez(filename, GPR_fit=gaussian_fit, empirical_nodes=self.empirical_nodes_idx, residual_greedy_basis=self.residual_greedy_basis, time=self.time, lml_fits=lml_fits, training_set=training_set, greedy_parameters_idx=self.greedy_parameters_idx, uncertainty_region=np.array(uncertainty_region, dtype=object))
    ...
    def generate_surrogate_model(self, plot_surr_datapiece_at_ecc=None, save_fig_datapiece=False, plot_surr_at_ecc=None, save_fig_surr=False, plot_GPRfit=False, save_fits_to_file=True, save_fig_fits=False, save_surr_to_file=False):
        ...
        def residual_to_original(residual_dataset, property):
            ...
            original_dataset = np.zeros((len(self.time), residual_dataset.shape[1]))

            for i in range(residual_dataset.shape[1]):
                if property == 'phase':
                    original_dataset[:, i] = circ - residual_dataset[:, i]
                elif property == 'amplitude':
                    original_dataset[:, i] = residual_dataset[:, i] + circ
                else:
        ...
        def reconstruct_surrogate_datapiece(property, B_matrix, fit_matrix, plot_surr_datapiece_at_ecc=plot_surr_datapiece_at_ecc, save_fig_datapiece=save_fig_datapiece):
            ...
            try:
                filename = f'_Straindata/Surrogate_datapieces/Surrogate_datapieces_f_lower={self.f_lower}_f_ref={self.f_ref}_e=[{min(self.ecc_parameter_space_input)}_{max(self.ecc_parameter_space_input)}_N={len(self.ecc_parameter_space_input)}]_No={len(self.ecc_parameter_space_output)}_gp={self.min_greedy_error_phase}_ga={self.min_greedy_error_amp}_Ngp={self.N_greedy_vecs_phase}_Nga={self.N_greedy_vecs_amp}.npz'
                load_surrogate = np.load(filename)
                ...
                reconstructed_residual = np.zeros((len(self.time), len(self.ecc_parameter_space_output)))

                for i in range(m):
                    reconstructed_residual = B_matrix.T @ fit_matrix
            ...
            if isinstance(plot_surr_datapiece_at_ecc, float):
                try:
                    index_ecc_ref = np.where(self.ecc_parameter_space_output == plot_surr_datapiece_at_ecc)[0][0]
                except:
                    plot_surr_datapiece_at_ecc_asked = plot_surr_datapiece_at_ecc
                    plot_surr_datapiece_at_ecc = self.ecc_parameter_space_output[np.abs(self.ecc_parameter_space_output - plot_surr_datapiece_at_ecc).argmin()]
                    index_ecc_ref = np.where(self.ecc_parameter_space_output == plot_surr_datapiece_at_ecc)[0][0]
                    print(f'Eccentricity value {plot_surr_datapiece_at_ecc_asked} not in ouput parameterspace. Eccentricity calculated for closest existing value at e={plot_surr_datapiece_at_ecc}.')
                ...
                real_hp, real_hc = self.simulate_inspiral_mass_independent(plot_surr_datapiece_at_ecc)
                ...
                axs[0].plot(self.time, surrogate_datapiece.T[index_ecc_ref], linewidth=0.6, label=f'surrogate e = {plot_surr_datapiece_at_ecc}')
                # axs[0].plot(self.time, true_phase[index_ecc_ref], linewidth=0.6, label=f'Surrogate: e = {plot_surr_datapiece_at_ecc}')
                axs[0].plot(self.time, real_datapiece, linewidth=0.6, linestyle='dashed', label=f'true {property} e = {plot_surr_datapiece_at_ecc}')
                ...
                relative_error = abs(surrogate_datapiece.T[index_ecc_ref] - real_datapiece) / abs(real_datapiece)
                ...
                if save_fig_datapiece is True:
                    figname = f'Surrogate_{property}_ecc_ref={plot_surr_datapiece_at_ecc}_f_lower={self.f_lower}_f_ref={self.f_ref}_e=[{min(self.ecc_parameter_space_input)}_{max(self.ecc_parameter_space_input)}_Ni={len(self.ecc_parameter_space_input)}]_No={len(self.ecc_parameter_space_output)}_gp={self.min_greedy_error_phase}_ga={self.min_greedy_error_amp}_Ngp={self.N_greedy_vecs_phase}_Nga={self.N_greedy_vecs_amp}.png'
                    
                    # Ensure the directory exists, creating it if necessary and save
                    os.makedirs('Images/Surrogate_datapieces_fullPS', exist_ok=True)
                    fig_surrogate_datapieces.savefig('Images/Surrogate_datapieces_fullPS/' + figname)

                    print('Figure is saved in Images/Surrogate_datapieces_fullPS')
        ...
        self.surrogate_amp, computation_time_amp = reconstruct_surrogate_datapiece(property='amplitude', B_matrix=B_matrix_amp, fit_matrix=fit_matrix_amp, plot_surr_datapiece_at_ecc=plot_surr_datapiece_at_ecc)  
        ...
        self.surrogate_phase, computation_time_phase = reconstruct_surrogate_datapiece(property='phase', B_matrix=B_matrix_phase, fit_matrix=fit_matrix_phase, plot_surr_datapiece_at_ecc=plot_surr_datapiece_at_ecc)
        ...
        if self.waveforms_in_geom_units is False:
            ...
            surrogate_amp_SI, surrogate_phase_SI = surrogate_datapieces_from_NR_to_SI()

            h_surrogate = np.zeros((len(self.total_mass_range), len(self.ecc_parameter_space_output), len(self.time))) #
            h_surrogate = surrogate_amp_SI * np.exp(1j * surrogate_phase_SI)

        else:
            h_surrogate = np.zeros((len(self.time), len(self.ecc_parameter_space_output)), dtype=complex)
        ...
        if isinstance(plot_surr_at_ecc, float):
            ...
            try:
                index_ecc_ref = np.where(self.ecc_parameter_space_output == plot_surr_at_ecc)[0][0]
            except:
                plot_surr_at_ecc_asked = plot_surr_at_ecc
                plot_surr_at_ecc = self.ecc_parameter_space_output[np.abs(self.ecc_parameter_space_output - plot_surr_at_ecc).argmin()]
                index_ecc_ref = np.where(self.ecc_parameter_space_output == plot_surr_at_ecc)[0][0]
                print(f'Eccentricity value {plot_surr_at_ecc_asked} not in ouput parameterspace. Eccentricity calculated for closest existing value at e={plot_surr_at_ecc}.')
            ...
            if self.waveforms_in_geom_units is True:
                true_hp, true_hc = self.simulate_inspiral_mass_independent(plot_surr_at_ecc)
            ...
            axs[0].plot(self.time, np.real(true_h), linewidth=0.6, label=f'true waveform e = {plot_surr_at_ecc}')
            axs[0].plot(self.time, np.real(h_surrogate[:, index_ecc_ref]), linewidth=0.6, label=f'surrogate e = {plot_surr_at_ecc}')
            axs[0].set_ylabel('$h_+$')
            axs[0].grid(True)
            axs[0].legend()

            # Calculate and Plot plus polarisation error 
            relative_error_hp = abs(np.real(h_surrogate[:, index_ecc_ref]) - np.real(true_h)) / abs(np.real(true_h))
            relative_error_hp[relative_error_hp > 1] = 0
            
            axs[1].plot(self.time, abs(np.real(h_surrogate[:, index_ecc_ref]) - np.real(true_h)), linewidth=0.6)
            ...
            axs[2].plot(self.time, np.imag(true_h), linewidth=0.6, label=f'true waveform e = {plot_surr_at_ecc}')
            axs[2].plot(self.time, np.imag(h_surrogate[:, index_ecc_ref]), linewidth=0.6, label=f'surrogate e = {plot_surr_at_ecc}')
            axs[2].grid(True)
            ...
            relative_error_hc = abs(np.imag(h_surrogate[:, index_ecc_ref]) - np.imag(true_h)) / abs(np.imag(true_h))
            relative_error_hc[relative_error_hc > 1] = 0
            axs[3].plot(self.time, abs(np.imag(h_surrogate[:, index_ecc_ref]) - np.imag(true_h)), linewidth=0.6)
            ...
            if save_fig_surr is True:
                figname = f'Surrogate_wf_ecc_ref={plot_surr_at_ecc}_f_lower={self.f_lower}_f_ref={self.f_ref}_e=[{min(self.ecc_parameter_space_input)}_{max(self.ecc_parameter_space_input)}_Ni={len(self.ecc_parameter_space_input)}]_No={len(self.ecc_parameter_space_output)}_gp={self.min_greedy_error_phase}_ga={self.min_greedy_error_amp}_Ngp={self.N_greedy_vecs_phase}_Nga={self.N_greedy_vecs_amp}.png'
        ...
        return h_surrogate, self.surrogate_amp, self.surrogate_phase, computation_time_amp, computation_time_phase
...
h_surrogate, surrogate_amp, surrogate_phase, computation_time_amp, computation_time_phase = gs.generate_surrogate_model(plot_surr_datapiece_at_ecc=0.2, plot_surr_at_ecc=0.2, plot_GPRfit=True, save_fig_datapiece=True, save_fig_surr=True, save_fits_to_file=True, save_surr_to_file=True, save_fig_fits=True)

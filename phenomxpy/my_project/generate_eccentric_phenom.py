...
class Simulate_Inspiral:
    ...
    def __init__(self, eccmin, time_array, mass_ratio=1, freqmin=650, chi1=0, chi2=0, rel_anomaly=0, inclination=0, distance=400., phi_ref=0, waveform_size=None):
        """
        Parameters:
        ----------------
        ...
        self.waveform_size = len(time_array) # Waveform size used for Surrogate model. Amount of indices before merger.
    ...
    def simulate_inspiral_mass_independent(self, eccmin=None, plot_polarisations=False, save_fig=False):
        ...
        # phen = PhenomT(mode=[2,2], eccentricity=ecc)
        phen.mass_1 = 1
        phen.mass_2 = 1
        phen.spin_1z = self.chi1
        phen.spin_2z = self.chi2
        phen.distance = self.distance
        phen.inclination = self.inclination
        phen.phi_ref = self.phi_ref
        phen.f_min = self.freqmin
        phen.delta_t = self.DeltaT

        # Time array same as LAL
        phen.compute_hlm(self.time)
        phen.compute_polarizations(self.inclination, self.phi_ref, self.time)

        # Make indepedent of mass and distance
        hp_Phenom_scaled = phen.hp * (self.distance / self.total_mass)
        hc_Phenom_scaled = phen.hc * (self.distance / self.total_mass)

        # Convert to TimeSeries for eccentric case
        hp_TS = types.timeseries.TimeSeries(phen.hp, delta_t=self.DeltaT, dtype=np.float32)  # plus polarisation in TimeSeries
        hc_TS = types.timeseries.TimeSeries(phen.hc, delta_t=self.DeltaT, dtype=np.float32)  # cross polarisation in TimeSeries
        t_TS = types.timeseries.TimeSeries(self.time, delta_t=self.DeltaT, dtype=np.float32) # Timeseries time-domain
        
        del phen.hp, phen.hc, self.time # Clear large variables after usage

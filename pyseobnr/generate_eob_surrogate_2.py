...
class Generate_Surrogate(Generate_TrainingSet):
    ...
    def fit_to_training_set(self, property, min_greedy_error=None, N_greedy_vecs=None, save_fits_to_file=True, plot_kernels=False, plot_fits=False, save_fig_kernels=False, save_fig_fits=False):
        ...
        if plot_fits is True:
            ...
            # Plotting data
            for i in range(len(gaussian_fit[-2:])):
                # Plot Gaussian fit
                ...
                # scatter_points = axs[0].scatter(self.parameter_space_input[self.greedy_parameters_idx], training_set[:, i], s=6)
               
                # Plot residuals (true property)
                strange, = axs[0].plot(self.parameter_space_output, residual_parameterspace_output[:, self.empirical_nodes_idx[i]], 
                ...
                dynamic_handles.append(strange)
    ...
    def generate_surrogate_model(self, plot_surr_datapiece_at_ecc=False, save_fig_datapiece=False, plot_surr_at_ecc=False, save_fig_surr=False, plot_GPRfit=False, save_fits_to_file=True, save_fig_fits=False, save_surr_to_file=False):
...
gs = Generate_Surrogate(parameter_space=[0.01, 0.3], amount_input_wfs=40, amount_output_wfs=100, N_greedy_vecs_amp=20, N_greedy_vecs_phase=20, freqmin=650)

gs.fit_to_training_set(property='phase', N_greedy_vecs=20, plot_fits=True, save_fig_fits=True, save_fits_to_file=True)
plt.show()
